...
function generateBoard(serverSeed: string, numRows: number = 12): number[] {
  const rows: number[] = [];
...
const StartSession = async (req: any, res: any) => {
  try {
    ...
    if (!master) return res.status(500).json({ success: false, message: "Server misconfigured" });
...
const RevealSession = async (req: any, res: any) => {
  try {
    ...
    if (!sessionDataStr) return res.status(404).json({ success: false, message: "Session not found" });
    
    const s = JSON.parse(sessionDataStr);
    if (s.status !== "open") return res.status(409).json({ success: false, message: "Already revealed" });
...
const GetProof = async (req: any, res: any) => {
  try {
    const sessionId = req.params.id?.toString();
    if (!sessionId) return res.status(400).json({ success: false, message: "Missing sessionId" });
...
const RestoreSession = async (req: any, res: any) => {
  try {
    const sessionId = req.params.id?.toString();
    if (!sessionId) return res.status(400).json({ success: false, message: "Missing sessionId" });
