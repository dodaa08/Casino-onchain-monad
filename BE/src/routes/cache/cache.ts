 ...
 */
const GetDeathIndex = async (req: any, res: any) => {
  const { sessionId, rowIndex } = req.params;
  try {
    ...
    const deathTiles = JSON.parse(deathTilesStr) as number[];
 ...
 */
const RevealSeed = async (req: any, res: any) => {
  const { sessionId } = req.params;
  try {
