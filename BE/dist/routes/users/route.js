    ...
    try {
        ...
        if (existing) {
            if (existing.balance !== balance) {
                const updatedUser = await User.findOneAndUpdate({ walletAddress }, { $set: { balance } }, { new: true });
...
const getAllUsersWithtotalEarned = async (req, res) => {
    try {
...
const getReferredUser = async (req, res) => {
